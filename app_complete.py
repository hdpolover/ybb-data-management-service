"""
YBB Data Management Service - Complete Production App
Handles numpy/pandas gracefully while maintaining full API functionality
"""
...
try:
    ...
    print("✅ Pandas and Excel libraries loaded successfully")
except ImportError as e:
    print(f"⚠️ Warning: Advanced libraries not available: {str(e)}")
...
try:
    ...
    print("✅ Full service modules loaded successfully")
except ImportError as e:
    print(f"⚠️ Warning: Service modules not available: {str(e)}")
...
if FULL_SERVICES_AVAILABLE:
    try:
        ...
        logger.info("✅ Full YBB Export Service initialized")
    except Exception as e:
        logger.warning(f"⚠️ Full service failed, using fallback: {str(e)}")
        FULL_SERVICES_AVAILABLE = False
...
@app.route('/api/ybb/storage/info', methods=['GET'])
def storage_info():
    """Get storage information"""
    try:
        # Use full service if available
        if FULL_SERVICES_AVAILABLE and hasattr(export_service, 'get_storage_info'):
            try:
                info = export_service.get_storage_info()
                return jsonify(info)
            except Exception as e:
                logger.warning(f"Full service storage info failed: {str(e)}")
        
        # Fallback implementation
        total_exports = len(app.exports_storage)
        total_size = sum(len(export_info.get("file_content", b"")) for export_info in app.exports_storage.values())
        
        return jsonify({
            "total_exports": total_exports,
            "total_files": total_exports,
            "total_size_bytes": total_size,
            "total_size_mb": round(total_size / (1024 * 1024), 2),
            "max_concurrent_exports": 20,
            "service_mode": "full_service" if FULL_SERVICES_AVAILABLE else "fallback",
            "request_id": g.request_id
        })
